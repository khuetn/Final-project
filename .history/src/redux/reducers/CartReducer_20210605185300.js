...
const CartReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case "INC":
      ...
      const discount = incState.products[indexInc].discount ? incState.products[indexInc].discount : 0
      incState.totalPrice = incState.totalPrice + incState.products[indexInc];
      incState.totalQuantities = incState.totalQuantities + 1;
      incState.products[indexInc] += 1;
      return incState;
    case "DEC":
      let decState = { ...state };
      let indexDec = decState.products.findIndex((pr) => pr.id === action.payload);
