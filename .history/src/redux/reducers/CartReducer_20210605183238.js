...
const CartReducer = (state = initialState, action) => {
  ...
  switch (action.type) {
    ...
    case "DEC":
      let decState = { ...state };
      let index = decState.products.findIndex((pr) => pr.id === product.id);
      if (decState.products[index].quantity > 1){

        decState.totalPrice =
          decState.totalPrice +
          (quantity - decState.products[index].quantity) * actualPrice;
        decState.totalQuantities =
          decState.totalQuantities +
          (quantity - decState.products[index].quantity) * actualPrice;
        decState.products[index] += quantity;
        return decState;
      }
    case "REMOVE":
      ...
      removeState.totalPrice =
        removeState.totalPrice - removeState.products[index].actualPrice;
